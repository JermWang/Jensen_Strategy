...
async function resumeDistributing(epoch) {
  ...
  return {
    ...
    epoch,
    processed_batches: 0,
    remaining_batches: batches.filter((batch) => batch.status !== "completed").length
  };
}
...
async function epochTick(options = {}) {
  ...
  try {
    ...
    else if (epoch.status === "snapshotting" || epoch.status === "distributing") result = await resumeDistributing(epoch);
    else {
