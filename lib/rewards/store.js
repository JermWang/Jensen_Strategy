...
function pool() {
  if (!globalThis.__btcPizzaRewardsPgPool) {
    ...
    globalThis.__btcPizzaRewardsPgPool = new Pool({
...
function localRoot() {
  return path.join(process.env.ADMIN_STORAGE_PATH ? path.resolve(process.env.ADMIN_STORAGE_PATH) : path.join(os.tmpdir(), "bitcoin-pizza-strategy-admin-data"), "rewards");
}
...
async function updateBatch(id, patch) {
  ...
  if (databaseConfigured()) {
    const sets = entries.map(([key], index) => `${key} = $${index + 2}`).join(", ");
    const values = entries.map(([, value]) => value);
    await query(`update reward_epoch_batches set ${sets}, updated_at = now() where id = $1`, [id, ...values]);
    return;
  }
  const data = readLocal();
