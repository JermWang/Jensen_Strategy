...
function holderPublic(row) {
  ...
  return {
    ...
    estimatedWbtc: Number(row.reward_ui || 0),
...
async function statusPayload() {
  ...
  return {
    ...
    total_wbtc_pool_raw: epoch?.total_reward_pool_raw || "0",
    total_wbtc_pool: formatUiAmount(epoch?.total_reward_pool_raw || "0", 8),
...
async function holdersPayload(wallet = "") {
  const config = rewardConfig();
  const epoch = await store.currentEpoch();
  const fallbackEpoch = epoch || (await store.latestEpoch());
  if (!fallbackEpoch) {
    return {
      ok: true,
      degraded: false,
  ...
  // Check if the stored epoch's token_mint matches current configuration
  const tokenMintMismatch = fallbackEpoch.token_mint && fallbackEpoch.token_mint !== config.tokenMint;
  if (tokenMintMismatch) {
    return {
      ok: true,
      degraded: true,
      reason: "token_mint_changed_awaiting_new_snapshot",
      message: `Token contract changed. Old: ${fallbackEpoch.token_mint?.slice(0, 8)}... Current: ${config.tokenMint?.slice(0, 8)}... Waiting for cron to capture new holders.`,
      epoch: epochPublic(fallbackEpoch),
      holder_cap: 5,
      cutoff_score: "0",
      eligible_shown: 0,
      snapshot_time: "",
      rows: [],
      holders: [],
      warning: "Holders will update when the next epoch closes with the new token contract."
    };
  }

  const rows = (await store.holdersForEpoch(fallbackEpoch.id)).map(holderPublic);
  const cutoff = rows.find((row) => row.in_holder_cap && row.rank === Number(fallbackEpoch.holder_cap)) || rows.filter((row) => row.in_holder_cap).at(-1);
  ...
  const current = query ? mapped.find((row) => row.wallet.toLowerCase() === query || row.wallet.toLowerCase().includes(query)) || null : null;
  return {
    ok: true,
    degraded: false,
    configured: true,
    source: fallbackEpoch.snapshot_source || "db-snapshot",
    sourceLabel: fallbackEpoch.snapshot_status === "snapshot_completed" ? "Stored snapshot" : "Snapshot pending",
    epoch: epochPublic(fallbackEpoch),
    roundCap: Number(fallbackEpoch.holder_cap),
    holder_cap: Number(fallbackEpoch.holder_cap),
    cutoffScore,
    ...
    snapshot_time: fallbackEpoch.snapshot_completed_at || fallbackEpoch.updated_at || "",
    updatedAt: fallbackEpoch.snapshot_completed_at || fallbackEpoch.updated_at || "",
    snapshot_slot: fallbackEpoch.snapshot_slot ?? null,
    current,
    rows: mapped,
    holders: mapped
  };
