...
async function lookupWallet(wallet) {
  ...
  if (!found) {
    return {
      ...
      reason: board.rows.length ? "not_in_top_128" : "no_snapshot_yet",
      message: board.rows.length
        ? "This wallet was not in the top 128 holder snapshot for this epoch."
  ...
  return {
    ...
    reason: found.in_holder_cap ? "eligible" : "not_in_top_128"
  };
}
