...
function readCreatorKeypair() {
  if (process.env.CREATOR_PRIVATE_KEY_BASE58) {
    return Keypair.fromSecretKey(decodeBase58(process.env.CREATOR_PRIVATE_KEY_BASE58));
  }

  if (process.env.CREATOR_KEYPAIR_PATH) {
    const raw = fs.readFileSync(process.env.CREATOR_KEYPAIR_PATH, "utf8").trim();
    if (raw.startsWith("[")) return Keypair.fromSecretKey(Uint8Array.from(JSON.parse(raw)));
    return Keypair.fromSecretKey(decodeBase58(raw));
  }

  throw new Error("Set CREATOR_PRIVATE_KEY_BASE58 or CREATOR_KEYPAIR_PATH for live creator-fee claims.");
}
...
function creatorPublicKey() {
  ...
  if (process.env.CREATOR_KEYPAIR_PATH || process.env.CREATOR_PRIVATE_KEY_BASE58) {
...
async function buildCreatorFeeClaimTransaction({ payload = {}, publicKey }) {
  ...
  if (body.mint || process.env.PUBLIC_TOKEN_MINT) requestBody.mint = body.mint || process.env.PUBLIC_TOKEN_MINT;
...
async function claimCreatorFees(payload = {}) {
  const dryRun = payload.dryRun !== false || process.env.CREATOR_FEE_DRY_RUN !== "false";
