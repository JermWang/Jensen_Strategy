...
async function claimCreatorFees(payload = {}) {
  ...
  // Capture pre-claim SOL balance so we can report the exact net amount the
  // claim added to the wallet. The epoch runner uses this number as the input
  // for the Jupiter SOL→NVDAx swap, so we buy NVDAx with exactly what
  // pump.fun just paid out (minus the gas cost of the claim itself).
  //
  // Both measurements must succeed for the delta to be trustworthy. If either
  // RPC call fails we report lamportsClaimed: 0 and let the caller decide
  // whether to fall back to MAX_CYCLE_SPEND_UI_AMOUNT — much safer than
  // accidentally treating a missed measurement as "the entire wallet is fees".
  let preBalanceLamports = null;
  try {
    preBalanceLamports = await connection.getBalance(keypair.publicKey, "confirmed");
  } catch {
    // pre measurement failed
  }
  ...
  let postBalanceLamports = null;
  try {
    postBalanceLamports = await connection.getBalance(keypair.publicKey, "confirmed");
  } catch {
    // post measurement failed
  }

  const measuredOk = preBalanceLamports !== null && postBalanceLamports !== null;
  const lamportsClaimed = measuredOk ? Math.max(0, postBalanceLamports - preBalanceLamports) : 0;
  ...
  return {
    ...
    apiUrl: built.apiUrl,
    request: built.requestBody,
    preBalanceLamports: preBalanceLamports ?? 0,
    postBalanceLamports: postBalanceLamports ?? 0,
    balanceMeasurementOk: measuredOk,
    lamportsClaimed,
    solClaimed: lamportsClaimed / 1_000_000_000
  };
}
