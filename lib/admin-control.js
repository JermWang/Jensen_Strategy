...
const { distributeNvdaxBatch } = require("./nvdax-distributor");
...
const ACTIONS = [
  ...
  },
  {
    ...
    requires: ["rpcConfigured", "distributorWallet", "nvdaxMint"],
    builtin: checkNvdaxVault
  },
  {
    id: "official-live-go",
    ...
    requires: ["rpcConfigured", "feeWallet", "tokenMint", "distributorWallet", "nvdaxMint"],
  ...
  },
  {
    ...
    webhookEnv: "ADMIN_QUOTE_NVDAX_BUY_WEBHOOK_URL",
    requires: ["nvdaxMint"],
    builtin: quoteNvdaxBuy
  },
  {
    id: "approve-nvdax-buy",
    ...
    webhookEnv: "ADMIN_APPROVE_NVDAX_BUY_WEBHOOK_URL",
    builtin: approveNvdaxBuy,
    dangerous: true
  },
  {
    id: "execute-nvdax-buy",
    label: "Buy NVDAx",
    group: "Money Ops",
    ...
    webhookEnv: "ADMIN_EXECUTE_NVDAX_BUY_WEBHOOK_URL",
    requires: ["nvdaxMint"],
    builtin: executeNvdaxBuy,
    dangerous: true
  },
  {
    id: "record-receipt",
  ...
  },
  {
    ...
    webhookEnv: "ADMIN_DISTRIBUTE_NVDAX_WEBHOOK_URL",
    requires: ["rpcConfigured", "nvdaxMint"],
    builtin: distributeNvdax,
    dangerous: true
  },
  {
...
function liveAutomationMissing(config = publicConfig()) {
  return [
    ...
    ["REWARD_MINT", Boolean(config.nvdaxMint)],
...
async function adminStatus() {
  ...
  if (automation?.startedAt) config.distributionStartedAt = automation.startedAt;
  const required = [
    ["ADMIN_PASSWORD or ADMIN_API_TOKEN", adminAuthConfigured()],
    ["SOLANA_RPC_URL", config.rpcConfigured],
    ["TOKEN_MINT", Boolean(config.tokenMint)],
...
async function checkNvdaxVault() {
  const config = publicConfig();
...
async function quoteNvdaxBuy(payload = {}) {
  const config = publicConfig();
  return await jupiterQuote({
    outputMint: config.nvdaxMint,
    payload
  });
}

async function approveNvdaxBuy() {
  return {
    configured: true,
    provider: "jupiter",
    status: "not_required",
...
async function executeNvdaxBuy(payload = {}) {
  const config = publicConfig();
  const body = payload.payload || {};
...
function distributionOptions(payload = {}) {
  ...
  return {
    ...
    rewardPool: Number(body.rewardPoolNvdax ?? body.rewardPool ?? 0),
...
function manifestMatchesCurrentToken(manifest, config = publicConfig()) {
  return Boolean(
    manifest &&
      ...
      (!manifest.nvdaxMint || sameAddress(manifest.nvdaxMint, config.nvdaxMint))
  );
}
...
function batchMatchesCurrentToken(batch, manifest, config = publicConfig()) {
  ...
  if (batch.nvdaxMint && !sameAddress(batch.nvdaxMint, config.nvdaxMint)) return false;
...
async function distributeNvdax(payload = {}) {
  const config = publicConfig();
  const body = payload.payload || {};
  ...
  if (result.signature) {
    await saveRecord("receipts", {
      type: "receipt",
      receiptType: "nvdax-distribution",
    ...
    try {
      ...
      for (const recipient of recipients) {
        await rewardsStore.saveReceipt({
          ...
          solscanUrl,
          status: "confirmed",
          notes: `nvdax-distribution batch ${batch.batchIndex || 0}`
        });
      }
    } catch {
...
async function runScheduledEpoch({ force = false, source = "manual", payload = {} } = {}) {
  ...
  try {
    ...
    // Decide how much SOL to swap into NVDAx this epoch.
    //
    // Default rule: swap the SOL that the creator-fee claim just added to the
    // wallet, MINUS a reserve to cover the upcoming Jupiter swap gas + every
    // distribution transaction (~5,000 lamports each) + new-recipient ATA
    // rent (~2,040,000 lamports per holder who hasn't received NVDAx before).
    //
    // The reserve also guarantees the wallet never drops below the reserve
    // amount after the swap, even if the claim was bigger than the carryover.
    //
    // Fallbacks:
    //   • If the claim returned 0 lamports (PumpPortal failure, no fees due,
    //     measurement failed) — fall back to MAX_CYCLE_SPEND_UI_AMOUNT.
    //   • If MAX_CYCLE_SPEND_UI_AMOUNT is also set, treat it as a hard cap on
    //     the swap so a freak huge claim can't drain everything.
    //   • If the post-reserve amount is below the dust floor, skip the swap —
    //     the SOL stays in the wallet and folds into next epoch's claim.
    const claimedLamports = Number(claim?.lamportsClaimed || 0);
    const postClaimBalanceLamports = Number(claim?.postBalanceLamports || 0);
    // Default reserve: 0.02 SOL. Enough for ~5–10 new-recipient ATA creations
    // plus several rounds of transfer-batch gas. Override with
    // EPOCH_SOL_RESERVE_LAMPORTS as your holder count grows (each new holder
    // who hasn't received NVDAx before costs ~0.00204 SOL of ATA rent).
    const reserveLamports = Number(process.env.EPOCH_SOL_RESERVE_LAMPORTS || 20_000_000);
    const fallbackSol = Number(process.env.MAX_CYCLE_SPEND_UI_AMOUNT || 0);
    const fallbackLamports = fallbackSol > 0 ? Math.floor(fallbackSol * 1_000_000_000) : 0;

    // Spendable from this epoch's claim — never let the post-swap wallet
    // balance dip below `reserveLamports`. When we know the wallet balance
    // right after the claim, use that as the hard ceiling. Otherwise spend
    // the claim minus the reserve as a best-effort guard.
    const spendableFromClaim = claimedLamports > 0
      ? (postClaimBalanceLamports > 0
          ? Math.max(0, Math.min(claimedLamports, postClaimBalanceLamports - reserveLamports))
          : Math.max(0, claimedLamports - reserveLamports))
      : 0;

    // Apply optional ceiling cap.
    const rawSwapLamports = spendableFromClaim > 0
      ? (fallbackLamports > 0 ? Math.min(spendableFromClaim, fallbackLamports) : spendableFromClaim)
      : fallbackLamports;

    // Floor — below this, the swap isn't worth the Jupiter gas + slippage and
    // would likely fail with "amount too small". Default 0.001 SOL.
    const minSwapLamports = Number(process.env.MIN_CYCLE_SPEND_LAMPORTS || 1_000_000);
    const swapLamports = rawSwapLamports >= minSwapLamports ? rawSwapLamports : 0;
    ...
    if (swapLamports > 0) {
      const buyPayload = {
        ...
        payload: {
          ...epochPayload.payload,
          inputAmountLamports: String(swapLamports),
          // Provenance for audit log so it's obvious which rule fired.
          swapInputSource: claimedLamports > 0
            ? (fallbackLamports > 0 && spendableFromClaim > fallbackLamports
                ? "claim_capped_by_max_cycle_spend"
                : "claimed_creator_fees_net_reserve")
            : "max_cycle_spend_fallback",
          claimedLamports: String(claimedLamports),
          postClaimBalanceLamports: String(postClaimBalanceLamports),
          reserveLamports: String(reserveLamports),
          spendableFromClaim: String(spendableFromClaim),
          maxCycleSpendLamports: String(fallbackLamports)
        }
      };
      buy = await runLaunchStep(steps, "execute-nvdax-buy", () => executeNvdaxBuy(buyPayload));
      if (buy?.signature) {
        await runLaunchStep(steps, "record-nvdax-buy-receipt", () =>
          recordReceipt({
            ...epochPayload,
            payload: {
              ...epochPayload.payload,
              receiptType: "nvdax-buy",
      ...
      // Either nothing to spend, or below the dust floor — record a skipped step
      // so the audit log shows why no swap happened this epoch.
      let reason;
      let message;
      if (rawSwapLamports === 0 && claimedLamports === 0 && fallbackLamports === 0) {
        reason = "no_claimed_fees_and_no_max_cycle_spend_configured";
        message = "Skipped Jupiter swap: creator-fee claim returned 0 lamports and MAX_CYCLE_SPEND_UI_AMOUNT is unset.";
      } else if (rawSwapLamports === 0 && claimedLamports > 0 && claimedLamports <= reserveLamports) {
        reason = "claim_consumed_by_sol_reserve";
        message = `Skipped Jupiter swap: claim of ${claimedLamports} lamports is at or below the EPOCH_SOL_RESERVE_LAMPORTS=${reserveLamports} guard. SOL stays in the wallet for distribution gas + ATA rent and folds into next epoch's claim.`;
      } else {
        reason = "claim_below_min_cycle_spend_floor";
        message = `Skipped Jupiter swap: post-reserve amount of ${rawSwapLamports} lamports is below MIN_CYCLE_SPEND_LAMPORTS=${minSwapLamports}. Fees will accumulate and swap on a future epoch.`;
      }
      steps.push({
        name: "execute-nvdax-buy",
        ok: true,
        result: {
          skipped: true,
          reason,
          ...
          reserveLamports: String(reserveLamports),
          spendableFromClaim: String(spendableFromClaim),
          rawSwapLamports: String(rawSwapLamports),
          minSwapLamports: String(minSwapLamports)
        }
      });
    }

    const nvdaxVault = await runLaunchStep(steps, "check-nvdax-vault", () => checkNvdaxVault());
    const rewardPoolNvdax = Number(nvdaxVault.balance || 0);
    if (rewardPoolNvdax <= 0) {
...
function launchPayloadWithRewardPool(payload, nvdaxVault) {
  const body = payload.payload || {};
  const rewardPoolNvdax = Number(body.rewardPoolNvdax ?? body.rewardPool ?? nvdaxVault?.balance ?? 0);
  return {
    ...payload,
    payload: {
      ...body,
...
async function officialLiveGo(payload = {}) {
  ...
  await runLaunchStep(steps, "refresh-fee-receipts", () => refreshFeeReceipts(), { optional: true });
  await runLaunchStep(steps, "check-nvdax-vault", () => checkNvdaxVault(), { optional: true });
