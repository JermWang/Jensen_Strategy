...
const { buildDistributionPolicy, currentDistributionEpoch } = require("./distribution-policy");
...
const ACTIONS = [
  ...
  },
  {
    ...
    group: "Launch",
    description: "Runs the launch-day sequence from live checks through snapshot lock, batch generation, and WBTC send.",
    operatorUse: "Use dry run first, then switch off Dry run and enable Confirm live when launch is intentional.",
...
async function officialLiveGo(payload = {}) {
  ...
  return {
    ...
    message: distribution
      ? "Official Live GO completed through WBTC distribution."
      : "Official Live GO locked the manifest and prepared a batch. Distribution was skipped by payload."
  };
}
