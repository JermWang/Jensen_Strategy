...
function configuredAdminSecrets() {
  return [process.env.ADMIN_PASSWORD, process.env.ADMIN_API_TOKEN].filter(Boolean);
}
...
function isAdminAuthorized(headers) {
  const expected = configuredAdminSecrets();
  ...
  return expected.some((secret) => safeEqual(direct, secret) || safeEqual(bearer, secret));
}

function adminAuthError() {
  return configuredAdminSecret()
    ? "Admin password is incorrect."
    : "ADMIN_PASSWORD or ADMIN_API_TOKEN is not configured in the server environment.";
}

function creatorClaimReady(config = publicConfig()) {
  return Boolean(config.creatorFeeClaimPublicKey || process.env.CREATOR_KEYPAIR_PATH || process.env.CREATOR_PRIVATE_KEY_BASE58);
}

function swapSignerReady() {
  return Boolean(
    process.env.JUPITER_SWAP_PRIVATE_KEY_BASE58 ||
      process.env.JUPITER_SWAP_KEYPAIR_PATH ||
      process.env.DISTRIBUTOR_KEYPAIR_PATH ||
      process.env.CREATOR_KEYPAIR_PATH
  );
}

function distributorSignerReady() {
  return Boolean(process.env.DISTRIBUTOR_PRIVATE_KEY_BASE58 || process.env.DISTRIBUTOR_KEYPAIR_PATH);
}
...
function liveAutomationMissing(config = publicConfig()) {
  return [
    ...
    ["CREATOR_PUBLIC_KEY or CREATOR_KEYPAIR_PATH", creatorClaimReady(config)],
    ["JUPITER_SWAP_PRIVATE_KEY_BASE58 or JUPITER_SWAP_KEYPAIR_PATH", swapSignerReady()],
    ["DISTRIBUTOR_PRIVATE_KEY_BASE58 or DISTRIBUTOR_KEYPAIR_PATH", distributorSignerReady()],
    ["CREATOR_FEE_DRY_RUN=false", process.env.CREATOR_FEE_DRY_RUN === "false"],
    ["DISTRIBUTOR_DRY_RUN=false", process.env.DISTRIBUTOR_DRY_RUN === "false"],
...
async function adminStatus() {
  ...
  const required = [
    ...
    ["PUBLIC_DISTRIBUTOR_WALLET", Boolean(config.distributorWallet)],
    ["PUBLIC_WBTC_MINT", Boolean(config.wbtcMint)],
    ["CREATOR_PUBLIC_KEY or CREATOR_KEYPAIR_PATH", creatorConfigured],
    ["JUPITER swap signer", swapSignerReady()],
    ["Distributor signer", distributorSignerReady()],
    ["CREATOR_FEE_DRY_RUN=false", process.env.CREATOR_FEE_DRY_RUN === "false"],
    ["DISTRIBUTOR_DRY_RUN=false", process.env.DISTRIBUTOR_DRY_RUN === "false"],
    ["CRON_SECRET", Boolean(process.env.CRON_SECRET)]
  ];
...
function tokenMismatchError(recordType, record, config = publicConfig()) {
  return new Error(
    `${recordType} ${record?.id || "record"} belongs to token ${record?.tokenMint || "unknown"}, but current PUBLIC_TOKEN_MINT is ${config.tokenMint}. Refresh holders and create a new snapshot before distributing.`
  );
}
...
async function distributeWbtc(payload = {}) {
  ...
  if (!batch) {
    throw new Error("No prepared distribution batch exists for the current token. Generate a current-token batch before executing WBTC distribution.");
  }
  const manifest = batch.manifestId ? await readRecord("manifests", batch.manifestId) : null;
  ...
  try {
    ...
    if (rewardPoolWbtc <= 0) {
      epochRecord = await saveRecord("epochs", {
        ...
        rewardPoolWbtc,
        steps,
        message: "Epoch completed without distribution because the WBTC pool is zero after fee claim and buy."
      });
    ...
    const batch = await runLaunchStep(steps, "generate-distribution-batch", () => generateDistributionBatch(batchPayload));
    const distribution = await runLaunchStep(steps, "distribute-wbtc", () =>
      distributeWbtc({
        ...distributionPayload,
        payload: {
          ...distributionPayload.payload,
    ...
    epochRecord = await saveRecord("epochs", {
      ...
      steps,
      message: "Epoch completed through creator-fee claim, WBTC buy, holder snapshot, and WBTC distribution."
    });
    const screenshot = await runLaunchStep(steps, "capture-dashboard-screenshot", () => captureEpochScreenshot(epochRecord), { optional: true });
    epochRecord = await saveRecord("epochs", {
      ...epochRecord,
...
async function officialLiveGo(payload = {}) {
  ...
  await runLaunchStep(steps, "refresh-fee-receipts", () => refreshFeeReceipts(), { optional: true });
  const wbtcVault = await runLaunchStep(steps, "check-wbtc-vault", () => checkWbtcVault());
  const launchPayload = launchPayloadWithRewardPool(payload, wbtcVault);
  const rewardPoolWbtc = Number(launchPayload.payload.rewardPoolWbtc || 0);
  ...
  if (rewardPoolWbtc <= 0) {
    return {
      ...
      rewardPoolWbtc,
      steps,
      message: "Launch is blocked because the WBTC reward pool is zero. Fund the distributor or enter a reward pool amount."
    };
  }

  if (dryRun) {
    await runLaunchStep(steps, "refresh-holder-list", () => refreshHolderList(launchPayload));
    const simulation = await runLaunchStep(steps, "simulate-distribution", () => simulateDistribution(launchPayload));
    return {
      configured: true,
      dryRun: true,
      startedAt,
      status: simulation.payableCount > 0 ? "ready" : "blocked",
      rewardPoolWbtc,
      payableCount: simulation.payableCount,
      batchCount: simulation.batchCount,
      steps,
      message:
        simulation.payableCount > 0
          ? "Dry-run launch path is ready. Switch off Dry run and enable Confirm live to execute."
          : "Dry-run launch found no payable recipients."
    };
  }

  const snapshot = await runLaunchStep(steps, "create-holder-snapshot", () => createHolderSnapshot(launchPayload));
  const manifestPayload = {
    ...launchPayload,
    payload: {
  ...
  if (simulation.payableCount <= 0) {
    return {
      ...
      rewardPoolWbtc,
      steps,
      message: "Live GO stopped before locking because there are no payable recipients."
    };
  }
  ...
  if (launchPayload.payload.executeDistribution !== false) {
    distribution = await runLaunchStep(steps, "distribute-wbtc", () =>
      distributeWbtc({
        ...launchPayload,
        payload: {
          ...launchPayload.payload,
  ...
  return {
    ...
    message: distribution
      ? "Official Live GO completed through WBTC distribution and armed continuous epochs."
      : "Official Live GO locked the manifest, prepared a batch, and armed continuous epochs. Distribution was skipped by payload."
  };
}
