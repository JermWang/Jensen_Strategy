...
const { databaseConfigured, databaseUrlSource, postgresPoolConfig } = require("./database");
...
function storageRoot() {
  ...
  if (isServerlessRuntime() && (defaultLocalPaths.has(configured) || configured.includes("/var/task"))) {
    return path.join(os.tmpdir(), "jensen-strategy-admin-data");
  }
...
function serverlessDatabaseError() {
  return new Error("SUPABASE_DATABASE_URL or DATABASE_URL is required for durable admin storage in serverless production.");
}

function pool() {
  if (!globalThis.__jensenStrategyAdminPgPool) {
    const { Pool } = require("pg");
    globalThis.__jensenStrategyAdminPgPool = new Pool(postgresPoolConfig());
  }
  return globalThis.__jensenStrategyAdminPgPool;
}
...
async function storageSummary() {
  ...
  return {
    ...
    root: databaseConfigured() ? `${databaseUrlSource()} env` : filesystemConfigured() ? storageRoot() : "SUPABASE_DATABASE_URL or DATABASE_URL required",
