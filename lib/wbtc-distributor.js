...
function readKeypair(filePath) {
  if (process.env.DISTRIBUTOR_PRIVATE_KEY_BASE58) {
    return Keypair.fromSecretKey(decodeBase58(process.env.DISTRIBUTOR_PRIVATE_KEY_BASE58));
  }

  if (!filePath) {
    throw new Error("DISTRIBUTOR_KEYPAIR_PATH or DISTRIBUTOR_PRIVATE_KEY_BASE58 is required for live WBTC distribution.");
  }

  const raw = fs.readFileSync(filePath, "utf8").trim();
  const bytes = raw.startsWith("[") ? Uint8Array.from(JSON.parse(raw)) : decodeBase58(raw);
...
async function distributeWbtcBatch({ batch, config, payload = {} }) {
  ...
  if (!hasSolanaRpc(process.env)) {
    throw new Error("SOLANA_RPC_URL or SOLANA_RPC_FALLBACK_URLS is required for WBTC distribution.");
  }
  ...
  const hasSigner = Boolean(process.env.DISTRIBUTOR_KEYPAIR_PATH || process.env.DISTRIBUTOR_PRIVATE_KEY_BASE58);
  const signer = dryRun && !hasSigner ? null : readKeypair(process.env.DISTRIBUTOR_KEYPAIR_PATH);
