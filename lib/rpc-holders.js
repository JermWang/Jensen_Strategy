...
function toDashboardSnapshot(snapshot, walletQuery = "", roundCap = 128) {
  const targetRoundCap = Math.max(1, Math.floor(Number(roundCap) || 128));
...
async function fetchRpcHolderSnapshot(options) {
  ...
  const [mintAccount, supply] = await Promise.all([
    rpc("getAccountInfo", [tokenMint, { encoding: "jsonParsed" }]),
  ...
  if (![TOKEN_PROGRAM_ID, TOKEN_2022_PROGRAM_ID].includes(tokenProgramId)) {
    throw new Error(`PUBLIC_TOKEN_MINT is not owned by a supported token program: ${tokenProgramId}`);
  }
...
async function fetchHeliusHolderSnapshot(options) {
  const { tokenMint, rpc, minBalanceUi = 0, excludedWallets = [] } = options;
  if (!tokenMint || tokenMint.length < 32) {
    throw new Error("PUBLIC_TOKEN_MINT is required for Helius holder snapshots.");
  }
...
async function fetchHolderSnapshot(options) {
  const provider = process.env.HOLDER_SNAPSHOT_PROVIDER || "solana-rpc";
  const fallbackEnabled = process.env.ENABLE_RPC_HOLDER_FALLBACK === "true";
  if (provider === "helius" || (provider === "auto" && heliusUrl())) {
    try {
